...
class GameCoopEnv(gym.Env):
    ...
    def step(self, action: ActType):
        ...
        reward, moving_avg = self.calc_inter_multi(res)
    ...
    def calc_inter_multi(self, param):
        """
        ...
        # Calculate percentage of heads meeting minimum immersion
        success_ratio = sum(1 for v in imms if v >= MIN_IMMERSIVENESS_TO_PASS) / len(imms)

        # Base reward on success ratio with additional components
        reward = 0.0
        reward += success_ratio  # Main component based on ratio of successful heads

        # Small bonus for very high immersion values
        # high_immersion_bonus = sum(0.1 for v in imms if v >= MIN_IMMERSIVENESS_TO_PASS * 1.2)
        # reward += high_immersion_bonus / len(imms)
        #reward += 0.01 # for conitnuity
        # Small penalty for very low immersion values
        # low_immersion_penalty = sum(0.1 for v in imms if v < MIN_IMMERSIVENESS_TO_PASS * 0.5)
        # reward -= low_immersion_penalty / len(imms)
         # Reward for taking more steps
    ...
    def calc_term_multi(self, termination_reason: str):
        ...
        print(f"\033[92mAll imm: {all_imm}, Avg imm: {avg_imm_msps}, Minimum requests done: {minimum_requests_done}, Final reward: {reward}\033[0m")
